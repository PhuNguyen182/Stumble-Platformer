...
public static class SimplePool
{
    ...
    public class Pool
    {
        ...
        public GameObject Spawn(Vector3 pos, Quaternion rot, Transform parent = null)
        {
            while (true)
            {
                ...
                if (_inactive.Count == 0)
                {
                    ...
                    // instantiate a whole new object.
                    obj = GameObject.Instantiate(_prefab, pos, rot, parent);
                    obj.name = string.Format("{0} ({1})", _prefab.name, _nextId++);
        ...
        {
            return Spawn(pos, rot, parent).TryGetComponent<T>(out var component) ? component : default;
        }
    ...
    {
        Init(prefab);
        GameObject bullet = _pools[prefab.GetInstanceID()].Spawn(pos, rot, parent);
        return bullet;
    }
    ...
    {
        Init(prefab.gameObject);
        T bullet = _pools[prefab.gameObject.GetInstanceID()].Spawn<T>(pos, rot, parent);
        return bullet;
    }
